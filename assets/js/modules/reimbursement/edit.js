...
const loadEditData = () => {
    const ref = normalizeDate(domEdit.reimbursementRef ? domEdit.reimbursementRef.value : '');
    if (!ref) {
        Swal.fire({
            icon: 'error',
            title: 'Error',
            text: 'No reimbursement reference provided.',
        }).then(() => {
    ...
    ajax_loader('transactions/reimbursement/api/get', { ReimbursementId: ref }).done((response) => {
        ...
        if (res.status !== 'success' || !res.data || !res.data.header) {
            Swal.fire({
                icon: 'error',
                title: 'Not Found',
                text: 'Reimbursement record not found.',
            }).then(() => {
                goToPath('transactions/reimbursement');
            });
            return;
        ...
        if (createdById !== currentUserId) {
            Swal.fire({
                icon: 'error',
                title: 'Access Denied',
                text: 'You can only edit your own reimbursements.',
            }).then(() => {
                goToPath('transactions/reimbursement');
            });
            return;
        ...
        if (statusCode !== 'RMB_SUBMITTED' && statusCode !== 'RMB_REJECTED') {
            Swal.fire({
                icon: 'warning',
                title: 'Cannot Edit',
                text: 'Only submitted or rejected reimbursements can be edited.',
            }).then(() => {
                goToPath('transactions/reimbursement');
            });
            return;
        ...
        if (domEdit.editDescription) domEdit.editDescription.textContent = normalizeDate(header.description || '') || '-';
        if (domEdit.editTotalAmount) domEdit.editTotalAmount.textContent = formatPHP(Number(header.total_amount || 0));

        // Load details with approval status
...
const getEditFormPayload = () => {
    ...
    const description = normalizeDate(domEdit.editDescription ? domEdit.editDescription.textContent : '');
...
const sendEditUpdate = (statusCode) => {
        ...
        ajax_loader_formdata_loading('transactions/reimbursement/api/update', formData).done((response) => {
            ...
            if (res.status !== 'success') {
                Swal.fire({
                    icon: 'error',
                    title: 'Failed',
                    text: res.response || 'Failed to update reimbursement.',
                });
                return;
            }

            const message = statusCode === 'RMB_DRAFT' ? 'Reimbursement saved as draft successfully.' : 'Reimbursement updated and resubmitted for approval.';
            const title = statusCode === 'RMB_DRAFT' ? 'Draft Saved' : 'Resubmitted';
    ...
    Swal.fire({
        ...
        text: 'Are you sure you want to resubmit this reimbursement for approval?',
        showCancelButton: true,
...
const cacheEditDom = () => {
    domEdit.reimbursementRef = document.getElementById('reimbursementRef');
    domEdit.editReimbursementNo = document.getElementById('editReimbursementNo');
    domEdit.editDescription = document.getElementById('editDescription');
    domEdit.editExpenseDate = document.getElementById('editExpenseDate');
    domEdit.editTotalAmount = document.getElementById('editTotalAmount');
            ...
            if (action === 'remove') {
                if (item && !item.isNew) {
                    Swal.fire({
                        icon: 'warning',
                        title: 'Remove Item?',
                        text: 'This item will be removed from the reimbursement.',
                        showCancelButton: true,
