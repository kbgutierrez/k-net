...
const loadRegister = (reset = false) => {
	...
	const request = ajax_loader('reports/disbursement-register/api/get', payload);
	...
	request.done((response) => {
		...
		Swal.fire({
			icon: 'error',
			title: 'Load Failed',
			text: 'Could not load the disbursement register.',
		});
	}).always(() => {
		isLoadingRows = false;
		updateLoadMoreButtons();
...
const renderDesktopTable = (rows) => {
	...
	if (!rows.length) {
		dom.registerTbody.innerHTML = '<tr><td colspan="10" class="text-center text-muted">No records found</td></tr>';
		return;
	}
	...
	rows.forEach((row) => {
		...
		item.innerHTML = `
			<div class="kna-row">
				...
				<div class="kna-small">${escapeHtml(getTypeLabel(row.transactionType))}</div>
			</div>
			...
			<div class="kna-row">
				<div class="kna-small text-muted">Released By</div>
				<div class="kna-small">${escapeHtml(row.actorName || '-')}</div>
			</div>
			<div class="kna-row">
				<div class="kna-small text-muted">Released Date</div>
...
const renderSummary = (rows) => {
	...
	const requesters = new Set(rows.map((r) => r.requesterName).filter(Boolean));
	dom.sumRequesters.textContent = String(requesters.size);

	const now = new Date();
	const ym = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}`;
	const thisMonthAmount = rows
		.filter((r) => toIsoDate(r.createdDate).startsWith(ym))
		.reduce((sum, r) => sum + r.amount, 0);
	dom.sumThisMonth.textContent = formatAmount(thisMonthAmount);
};
...
const downloadExcel = () => {
	...
	window.location.href = `${base_url}reports/disbursement-register/download/excel${query ? '?' + query : ''}`;
};
...
const cacheDom = () => {
	...
	dom.sumRequesters = document.getElementById('sumRequesters');
	dom.sumThisMonth = document.getElementById('sumThisMonth');
